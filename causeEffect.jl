...
function explore(a, m1, m2, b, _view::SubArray)
    ...
    b != lower ? effect(lower, b, view(_view, lower:b))   : b ; println("Scalar Value = " , b)

    println("Scalar Value = ", c); #: #_view[b] #return

end

# let ar1 be equal to (custom)
# Part 2: define ar1 (to anything you'd like)
...
=#
 # Part3: TODO: define isStop()
#---test: define isStop (now, it is used as a `placeholder` so that it won't execute code, beyond this point)

""" checks if function has reached a stopping condition
At any rate, this is a fine function that it  will expose any further `Hidden Deformalities`, in the function Implementation:
possible common Issues:
1. More cycles (recursions)
2. Less cycles (than anticipated) """
# latest: ERROR: LoadError: MethodError: no method matching isStop(::Int64, ::Int64, ::SubArray{Int64, 1, Vector{Int64}, Tuple{UnitRange{Int64}}, true})
# TODO: Latest: define  : isStop(::Int64, ::Int64, ::SubArray{Int64, 1, Vector{Int64}, Tuple{UnitRange{Int64}}, true})

function isStop(currentValue; exceptionParameter = UnexpectedError) #scaffold
    ...
    try
        ...
        elseif currentValue > 0 #1
            ...
            writeError(msg,  exceptionParameter ) #throw(error(msg)) # severe Error
        end
        return 0 # end
...
#=
isStop(1, 2, view(arr, 1:2) ) == true ? endAlgorithmSafely(arr) : 0 #doOtherThing() # is stop (not stopping, yet) #TODO: endAlgorithmSafely
...
=#
# THE END ....
## Update: Part4:  ERROR: LoadError: MethodError: no method matching goright!(::Vector{Int64})
...
#2. `StoppingCondition` (not defined) #TODO: Define : stoppingCondition

#goright!(firstindex(arr), lastindex(arr), arr) #does Nothing #Warning! #funcs: checkCond

##  goright!(arr) # #does the sorting #<--- still, here : finally:
# ERROR: LoadError: UndefVarError: stoppingCondition not defined
# TODO: define stoppingCondition()
...
# Part 6:
#=
ERROR: LoadError: MethodError: swapContent(::Int64, ::Int64, ::Vector{Int64}) is ambiguous. Candidates:
  swapContent(aContent, bContent, arr::Vector{Int64}) in Main at C:\Users\ASUS\Documents\New folder\CauseEffect-main\Julia\src\causeEffect.jl:1201
  swapContent(aContent::Int64, bContent::Int64, _view) in Main at C:\Users\ASUS\Documents\New folder\CauseEffect-main\Julia\src\causeEffect.jl:143
Possible fix, define
  swapContent(::Int64, ::Int64, ::Vector{Int64})
# The real End
=#
println(arr ) #or unCommentlines below , accordingly
...
function isStoppingCondition(arr::Array{Int64,1}, currentValue) # =2::Int64) #vital
    ...
    if isStop(currentValue) == 0 #useful for a recursion #check if done
        ...
        #instead
        #callMiddle!(a, m1,) #TODO: complete this logic
        #create view
        #callMiddle!(a,b,view)
        #isWhole == true ? callMiddle(m1, b) : callMiddle(m2, b)
        return false
